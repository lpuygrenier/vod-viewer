// Import the modules (in a real ES6 environment, these would be import statements)
class DrawingCanvas {
  constructor() {
...
const uiManager = new UIManager(videoManager, nameManager);

// Global functions for HTML event handlers
function importVideos() {
  uiManager.importVideos();
}
...
function goForward(seconds) {
  videoManager.seek(true, seconds);
}

function goBack(seconds) {
  videoManager.seek(false, seconds);
}
...
function toggleSync() {
  uiManager.toggleSync();
}

function updateFocus(video) {
  uiManager.updateFocus(video);
}
